...
class BGPNetwork:
    def __init__(self, verbose=False, interactive=False) -> None:
        self.AS: dict[int, nx.DiGraph] = {}
        self.network = nx.DiGraph()
        self.bgp_tables: dict[str, dict[str, set[BGPRoute]]] = {} # { router => { Prefix => {...Routes} } }
    ...
    def decision_process(self, router: str, prefix: str):
        routes = self.bgp_tables[router].get(prefix, set())
        best = None
        ...
        lowest_med: dict[int, list[BGPRoute]] = {}
        ...
        routes_filtered_med: list[BGPRoute] = []
        for r in lowest_med.values():
            routes_filtered_med = routes_filtered_med + r

        best = None
        for route in routes_filtered_med:
            if best is None:
                best = route
            elif route.src != best.src:
                best = best if best.src == "ebgp" else route
            elif route.src == "ibgp" and self.distance(
                router, route.nexthop
            ) != self.distance(router, best.nexthop):
                best = (
                    best
                    if self.distance(router, best.nexthop)
                    < self.distance(router, route.nexthop)
                    else route
                )
            else:
        ...
        if previous_best != best:
            ...
            for _ip, router in self.AS[AS].nodes(data=True):
                ...
                if previous_best != None and previous_best.src != "ibgp":
                    to_remove = BGPRoute(
                        previous_best.prefix,
                        f"10.0.{AS}.{current_id}",
                        previous_best.as_path,
                        previous_best.pref,
                        previous_best.med,
                        current_id,
                        src="ibgp",
            ...
            self.log(
                f"Router {current} has a new best route {new_route} to reach {route.prefix}"
            )
                ...
                if data["type"] != "internal":
                    ...
                    if previous_best != None:
                        to_remove = BGPRoute(
                            previous_best.prefix,
                            f"10.0.{AS}.{current_id}",
                            (AS,) + previous_best.as_path,
                            -1,
                            med,
                            -1,
                        )
                        self.log(
                            f"Router {current} withdraw route {to_remove} because new best route found"
                        )
                        self.announce_route(
                            to_remove, neigh, current, BGPMessage.WITHDRAW
                        )
                    if (
                        type_rel != "customer"
                        and self.network.adj[current][neigh]["type"] != "customer"
                    ):
    ...
    def withdraw(self, route: BGPRoute, current: str, origin: str):
        ...
        if best == route:
            ...
            if new_best == None:
                return
            to_remove = BGPRoute(
                best.prefix, f"10.0.{AS}.{current_id}", (AS,) + best.as_path, -1, -1, -1
            )
            ...
            for neigh, data in self.network.adj[current].items():
                if data["type"] != "internal":
                    ...
                    if new_best != None:
                        ...
                        self.log(
                            f"Router {current} announces new best route {to_announce}"
                        )
                        self.announce_route(
                            to_announce, neigh, current, BGPMessage.UPDATE
                        )

    def announce_route(
        self, route: BGPRoute, current: str, origin: str, type: BGPMessage
    ):
    ...
    def announce_prefix(self, router: str):
        ...
        route = BGPRoute(
            prefix, f"10.0.{data['AS']}.{data['id']}", (data["AS"],), 1000, 0, -1
        )
            ...
            for _ip, router in self.AS[AS].nodes(data=True):
                ...
                for prefix in self.bgp_tables[router]:
                    ...
                    for route in self.bgp_tables[router][prefix]:
                        ...
                        print(
                            f"nexthop={route.nexthop}, pref={route.pref:3d}, AS path={as_path}, MED={route.med}"
                        )
        print()
    ...
    def plot_network(self):
        ...
        peers = [
            (u, v) for u, v, e in self.network.edges(data=True) if e["type"] == "peer"
        ]
        providers = [
            (u, v)
