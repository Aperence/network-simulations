        ...
        match self.command_receiver.try_recv(){
            Ok(command) => {
                match command{
                    ...
                    Command::AddPeerLink(_, _, _, _) => panic!("Adding peer link not supported on switch"),
                    Command::AddProvider(_, _, _, _) => panic!("Adding provider link not supported on switch"),
                    Command::AddCustomer(_, _, _, _) => panic!("Adding customer link not supported on switch"),
